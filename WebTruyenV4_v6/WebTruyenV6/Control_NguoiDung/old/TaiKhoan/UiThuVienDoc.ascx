<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UiThuVienDoc.ascx.cs" Inherits="WebTruyenV4.Control_NguoiDung.TaiKhoan.UiThuVienDoc" %>
<script>
...
<section class="box_div maunen_div">
   <div class="div_title">
      <img src="<%=Request.Url.GetLeftPart(UriPartial.Authority) %>/public/img/www/top-icon/contact-list.png"  width="40"/>Thư viện <!-- da sua thanh 2 cham  -->
   </div>
      ...
      <div class="row list_truyen">
         ...
         <div class="col-6 col-sm-5 col-md-3 col-lg-3 col-xl-2" style="margin-top: 2rem;">
             ...
             <div class="card" style="background: #ff8d18;  padding: 1px;">
                <img title="Có Chương mới cập nhật !!!" style="position: absolute;" width="60" src="<%=Request.Url.GetLeftPart(UriPartial.Authority) %>/public/img/www/top-icon/New.gif" />
                <% }else{ %>
                 <div class="card">
                 <%} %>
               ...
               <!--  <p class="dis_tap">Tên Tập 1: aaaa</p> -->
                              <a href="<%=Request.Url.GetLeftPart(UriPartial.Authority) %>/truyen/<%=ThuVien[a].link_raw + "-" + ThuVien[a].id_truyen %>">
                  <!-- da sua thanh 2 cham  -->
                  <!--   <button class="time btn-danger">31 phút trước</button> -->
                  <img class="card-img-top" src="<%=Request.Url.GetLeftPart(UriPartial.Authority) %>/public/img/truyen/<%=ThuVien[a].img_truyen %>"onError="this.onerror=null;this.src='<%=Request.Url.GetLeftPart(UriPartial.Authority) %>/public/img/www/err-img.png';" /><!-- da sua thanh 2 cham  -->
               </a>
                 <a title="<%=TenChuong(Convert.ToInt32(ThuVien[a].id_chuong)) %>" href="<%=Request.Url.GetLeftPart(UriPartial.Authority) %>/truyen/<%=ThuVien[a].link_raw + "-" + ThuVien[a].id_truyen + "/" + ThuVien[a].id_chuong %>">
              <div class="tenchuong"><% RutGon_TieuDe(TenChuong(Convert.ToInt32(ThuVien[a].id_chuong)), 15); %></div>
                     </a>
               <div class="card-body">
                  <a title="<%=ThuVien[a].ten_truyen %>" href="<%=Request.Url.GetLeftPart(UriPartial.Authority) %>/truyen/<%=ThuVien[a].link_raw + "-" + ThuVien[a].id_truyen %>">
